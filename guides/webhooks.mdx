---
title: "Receive Procurement Approval Webhooks Securely"
...
app.post('/webhooks/procurement', (req, res) => {
  ...
  const { session_id, decision, actor } = req.body;
  // Resume or cancel the orchestration flow based on the decision
  res.status(204).send();
});
```
...
Use the `decision` field to branch your handler logic:
...
app.post('/webhooks/procurement', (req, res) => {
  ...
  const { session_id, decision, actor, reason } = req.body;
